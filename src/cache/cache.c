...
cache_line_t *get_line(cache_t *cache, uword_t addr) {
    /* your implementation */
    size_t S = cache->C / (cache->A * cache->B);
    size_t s = (size_t) _log(S);
    size_t b = (size_t) _log(cache->B);
    uword_t tag = (addr >> (b + s));
    uword_t set_index = (addr >> b) & (S - 1);
    cache_set_t *temp_set = cache->sets + set_index;

    for (int i = 0 ; i < cache->A; i++) {
        if (tag == temp_set->lines[i].tag && temp_set->lines[i].valid) {
...
cache_line_t *select_line(cache_t *cache, uword_t addr) {
    ...
    // addr & (~(~(0x0ULL) << (s_and_b))) >> _log(cache->B);
    ...
    for (int i = 0 ; i < cache->A; i++) {
        if (temp_set->lines[i].valid == 0) {
            return temp_set->lines + i;
        }

        if (temp_set->lines[i].lru < min_lru) {
            min_lru = temp_set->lines[i].lru;
            min_index = i;
        }
    }

    return temp_set->lines + min_index;
}
...
evicted_line_t *handle_miss(cache_t *cache, uword_t addr, operation_t operation, byte_t *incoming_data) {
    ...
    cache_line_t *temp_line = select_line(cache, addr);

    size_t S = cache->C / (cache->A * cache->B);
    size_t b = (unsigned int) _log((size_t) cache->B);
    size_t s = (unsigned int) _log((size_t) S);


    size_t index = (addr >> b) & (S - 1);

    evicted_line->dirty = temp_line->dirty;
    evicted_line->valid = temp_line->valid;
    temp_line->lru = next_lru++;
    evicted_line->addr = (temp_line->tag << (s + b)) | (index << b);
    ...
    temp_line->tag = (addr >> (b + s));
    temp_line->dirty = operation == 1;
    temp_line->valid = 1;

    memcpy(evicted_line->data, temp_line->data, cache->B);

    if (incoming_data != NULL) {
        memcpy(temp_line->data, incoming_data, cache->B);
    }
...
void get_word_cache(cache_t *cache, uword_t addr, word_t *dest) {
    /* Your implementation */
    cache_line_t *temp_line = get_line(cache, addr);
    char *temp_dest = (char*) dest;

    for (size_t i = 0; i < 8; i++) {
        temp_dest[i] = temp_line->data[addr % cache->B];
        addr++;
    }
}
...
void set_word_cache(cache_t *cache, uword_t addr, word_t val) {
    /* Your implementation */
    cache_line_t *temp_line = get_line(cache, addr);
    char *temp_dest = (char*) &val;

    for (size_t i = 0; i < 8; i++) {
        temp_line->data[addr % cache->B] = temp_dest[i];
        addr++;
    }
}
